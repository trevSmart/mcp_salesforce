		...
		messages: [
			{
				...
				content: {
					type: 'text',
					text: `Perform a safe and comprehensive test of the ${config.SERVER_CONSTANTS.serverInfo.name} tools, without modifying the org or workspace.
...
5) describeObject
   ...
   - includeFields: false (light test)
   - Optional second call: includeFields: true, includePicklistValues: false (if you want to validate fields without loading too much data)

6) apexDebugLogs (only non-mutating actions)
   - action: "status"
   - action: "list"
...
9) executeAnonymousApex (without persistent changes)
   - apexCode: "System.debug('MCP safe test ping');"
   - mayModify: false

10) runApexTest (optional, only if there are @isTest classes and to execute a single brief test)
...
Final summary: return a summary with the status of each call (success/error) and any relevant notes (e.g., how many records returned, first Id used, first log obtained, etc.).`
				}
			}
		]
	};
