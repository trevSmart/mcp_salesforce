 ...
 * @param {string} orgAlias - Àlies de l'org
 * @returns {Promise<Object>} - Resultat cru de la descripció
 */
export async function describeObjectService(sObjectName, orgAlias) {
	...
	if (!orgAlias || typeof orgAlias !== 'string') {
		throw new Error('orgAlias és obligatori i ha de ser una string');
	}
	try {
		const command = `sf sobject describe --sobject ${sObjectName} -o "${orgAlias}" --json`;
