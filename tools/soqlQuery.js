...
async function soqlQuery({query, useToolingApi = false}) {
	try {
		...
		const command = `sf data query --query "${cleanQuery.replace(/"/g, '\\"')}" -o ${process.env.username} ${toolingFlag} --json`;
		console.error(`Executing SOQL query command: ${command}`);
		const response = await runCliCommand(command);
		const records = response.result.records.map(r => ({...r, href: `https://${process.env.instance}.lightning.force.com/${r.Id}`}));
		return {
			content: [
			{
				type: 'text',
